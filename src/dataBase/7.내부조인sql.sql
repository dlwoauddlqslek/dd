...
#2. 다중 테이블의 데이터 검색: ,(쉼표) 이요한 테이블명 구분
	#select * from 테이블명1, 테이블명2
    #select * from 테이블명1, 테이블명2 where 테이블명1.필드명=조건값
    #select * from 테이블명1 별칭
		#별칭: 주로 테이블명,필드명을 간추려 SQL 질의어 에서 사용할 때
select * from table1, table2; # 레코드 25개, 첫번째이블개수 * 두번째테이블개수
select * from table1, table2 where num=num; #pk필드와 fk필드 식별 불가능해서 오류 발생
select * from table1, table2 where table1.num=table2.num; #5개
select * from table1 a, tabl2 b where a.num=b.num; #5개
#왜? PK와 FK 필드가 같은 경우를 검색하는지? 관계를 찾기 위해서

#3. JOIN 절
#3-1 교집합 -> 두 개 이상의 집합/테이블이 공통된 수/데이터 찾기, pk-fk 교집합
#[1] where 절
