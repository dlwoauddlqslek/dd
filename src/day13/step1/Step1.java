...
public class Step1 {
  public static void main(String[] args) {
    ...
    // 강제타입 변환 *전제조건 필요 : byte<-(byte)short<-(short)int
    // 부모 타입은 자식타입으로 변환 불가능, 단 상속관계이면 가능, 상하관계만 가능(형제는 불가능)
    // 조류 bird3=new 동물();
       참새 sparrow3=new 참새();
       동물 참새에서동물=sparrow3; //자식 -> 부모
       참새 동물에서참새=(참새)참새에서동물; // 부모 -> 자식

      닭 chicken=new 닭();
      동물 닭에서동물=chicken; //자식 -> 부모
      닭 동물에서닭=(닭)닭에서동물; //부모 -> 자식

      // 참새 닭에서참새=(참새)동물에서닭; //형제관계 불가능
    ...
    // 부모타입으로 배열/리스트를 선언하면 해당 배열/리스트에는 본인타입 포함 하위타입 저장 가능. 상위타입은 불가능
